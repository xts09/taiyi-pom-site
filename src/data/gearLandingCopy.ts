...
export const gearLandingCopy = {
  en: {
    ...
    caseSummary: `A massage-chair customer used ${test.grade} to address insufficient armrest gear wear resistance, then adopted the material for production and placed repeat orders. The customer-side validation recorded ${test.sampleQuantity} sample at ${test.loadPerSideKg}\u00a0kg per side, ${test.temperatureC}\u00a0±\u00a0${test.temperatureToleranceC}°C and ${test.relativeHumidityPercent}%\u00a0RH: the assembly met the ${cycles}-cycle requirement, with small-gear wear and slipping recorded in further testing. The test result applies to that sample and those conditions.`,
  ...
  zh: {
    ...
    caseSummary: `针对按摩椅扶手齿轮耐磨表现不足，客户采用 ${test.grade} 进行组件验证，随后进入量产并复购。客户侧 ${test.sampleQuantity} 件样件在左右各 ${test.loadPerSideKg}\u00a0kg、${test.temperatureC}\u00a0±\u00a0${test.temperatureToleranceC}°C、${test.relativeHumidityPercent}%\u00a0RH 条件下满足了 ${cycles}\u00a0次项目要求；继续运行后记录到小齿轮磨损打滑，试验结论适用于该次样件与工况。`,
    caseAction: "查看完整案例",
